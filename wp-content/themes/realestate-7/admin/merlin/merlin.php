...
class Merlin {
	...
	function steps() {
		...
		if ( $this->theme_license_step_enabled ) {
			$this->steps['edd-license'] = array(
				'name' => esc_html__( 'Theme license', 'merlin-wp' ),
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json( array(
				'success' => false,
				'message' => esc_html__( 'Access denied!', 'merlin-wp' ),
			) );
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json( array(
				'success' => false,
				'message' => esc_html__( 'Please input your license key!', 'merlin-wp' ),
			) );
		}
			...
			if ( false === $license_data->success ) {
				...
				switch( $license_data->error ) {
					...
					case 'missing' :
						...
						$message = esc_html__( 'Invalid license.', 'merlin-wp' );
						break;
				...
				if ( 'valid' === $license_data->license ) {
					$message = esc_html__( 'The license was successfully activated!', 'merlin-wp' );
					$success = true;
