        ...
        if (!isset($parameters['pagetoken']))
        {
            switch ($method)
            {
                case 'nearbysearch':
                    ...
                    {
                        echo '<div class="marT20 nomatches">';
                            echo '<h5>' . __('Google Places API is currently unavailable.', 'contempo') . '</h5>';
                            echo '<p class="marB0">';
                                _e('You must specify a valid listing location, use street address in the listing title along with city, state & zip/postcode — if not use the optional latitude & longitude field.', 'contempo');
                            echo '</p>';
                        echo '</div>';
                    }
                            ...
                            case 'distance':
                                    ...
                                    && !isset($parameters['types']))
                                {
                                    echo '<div class="marT20 nomatches">';
                                        echo '<h5>' . __('Google Places API is currently unavailable.', 'contempo') . '</h5>';
                                        echo '<p class="marB0">';
                                           _e('You much specify at least one of the following: "keyword", "name", "types".', 'contempo');
                                        echo '</p>';
                                    echo '</div>';
                                }
                            ...
                            case 'prominence':
                                if (!isset($parameters['radius']))
                                {
                                    echo '<div class="marT20 nomatches">';
                                        echo '<h5>' . __('Google Places API is currently unavailable.', 'contempo') . '</h5>';
                                        echo '<p class="marB0">';
                                            _e('You must specify a radius.', 'contempo');
                                        echo '</p>';
                                    echo '</div>';
                                }
                                break;
                ...
                case 'radarsearch':
                    if (!isset($parameters['location']))
                    {
                        _e('You must specify a location before calling radarsearch().', 'contempo');
                    }
                ...
                case 'details':
                    ...
                    {
                        _e('You must specify either a "placeid" or a "reference" (but not both) before calling details().', 'contempo');
                    }
