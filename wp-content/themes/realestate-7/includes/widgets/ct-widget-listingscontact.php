...
if(!function_exists('ct_ListingsContact')) {
	class ct_ListingsContact extends WP_Widget {
	   ...
	   function form($instance) {
	        ...
	        <p>
			   <label for="<?php echo esc_attr($this->get_field_id('subject')); ?>"><?php esc_html_e('Subject:','contempo'); ?></label>
			   <input type="text" name="<?php echo esc_attr($this->get_field_name('subject')); ?>"  value="<?php echo esc_attr($subject); ?>" class="widefat" id="<?php echo esc_attr($this->get_field_id('subject')); ?>" />
			</p>
			<?php
		}
	} 
