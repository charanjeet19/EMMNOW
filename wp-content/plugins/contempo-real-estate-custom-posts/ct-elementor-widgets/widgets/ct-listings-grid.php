	...
	protected function render() {
		...
		echo '<ul class="col span_12 row first">';
	        ...
	        if ( $wp_query->have_posts() ) : while ( $wp_query->have_posts() ) : $wp_query->the_post();
				...
				<li class="listing col <?php if($settings['number'] == '1') { ?>span_12<?php } else { ?>span_4<?php } ?> <?php echo $ct_search_results_listing_style; ?> <?php if($count % 3 == 0) { echo 'first'; } ?> <?php if(get_post_meta($post->ID, "source", true) == 'idx-api') { echo 'idx-listing'; } ?>">
		            <figure>
		            	<?php
